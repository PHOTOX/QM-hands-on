...
where
\begin{align*}
    % (\Delta x)^2 &= \braket{x^2} - \braket{x}^2 = \bra{\psi(x, t)}x^2\ket{\psi(x,t)} -  \bra{\psi(x, t)}x\ket{\psi(x,t)}^2\\    
    \Delta x &= \sqrt{\braket{x^2} - \braket{x}^2} = \sqrt{\bra{\psi(x, t)}x^2\ket{\psi(x,t)} -  \bra{\psi(x, t)}x\ket{\psi(x,t)}^2} \, , \\
    % (\Delta p)^2 &= \braket{p^2} - \braket{p}^2 = \bra{\psi(x, t)}\hat{p}^2\ket{\psi(x,t)} -  \bra{\psi(x, t)}\hat{p}\ket{\psi(x,t)}^2 \, .
    \Delta p &= \sqrt{\braket{p^2} - \braket{p}^2} = \sqrt{\bra{\psi(x, t)}\hat{p}^2\ket{\psi(x,t)} -  \bra{\psi(x, t)}\hat{p}\ket{\psi(x,t)}^2} \, .
\end{align*}
