...
alpha, x0, p0 = 0.1, 1.5, 0.0
psi0 = (2*alpha/np.pi)**0.25*np.exp(-alpha*(x - x0)**2 + 1j*p0*(x - x0), dtype=complex)

# create list for final optimized wave functions and energies
...
for state in range(nstates):
    ...
    # creating old energy to compare with new energy at every step
    ...
    print("\nLaunching imaginary-time quantum dynamics.\n------------------------------------------\n")
    while t < simtime:  # loop until simulation time is reached
        ...
        # renormalize the function so that the norm is one again
        psi /= np.sqrt(norm)
