...
xmin, xmax = -1.5, 1.5  # minimum and maximum of x
simtime = 3.0  # simulation time in atomic time units
...
alpha, x0, p0 = m*omega*2, -0.90, 0.0
# alpha, x0, p0 = m*omega*2, -0.95, 8.0
...
while t < simtime:  # loop until simulation time is reached
    ...
    p_arr.append(np.real(np.trapz(y=np.conjugate(psi[int(ngrid/2):])*psi[int(ngrid/2):], x=x[int(ngrid/2):])))
    t_arr.append(t)
...
plt.xlabel("Time (a.u.)")
plt.ylabel(f"Tunneling efficiency $\kappa(t)$")
plt.xlim(0, simtime)
plt.ylim(0, 1)
plt.show()
